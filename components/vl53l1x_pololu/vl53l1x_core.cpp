...
// millis()/delay()/delayMicroseconds() resolve to esphome::* via the enclosing
// namespace, so this works on both the arduino and esp-idf frameworks.
...
{
  uint8_t d = value;
  last_status = (this->io_->write_register16(reg, &d, 1) != i2c::ERROR_OK);
}
...
void VL53L1X::writeReg16Bit(uint16_t reg, uint16_t value)
{
  ...
  last_status = (this->io_->write_register16(reg, d, 2) != i2c::ERROR_OK);
}

// Write a 32-bit register
void VL53L1X::writeReg32Bit(uint16_t reg, uint32_t value)
{
  uint8_t d[4] = { (uint8_t)(value >> 24), (uint8_t)(value >> 16),
                   (uint8_t)(value >> 8),  (uint8_t)(value) };
  last_status = (this->io_->write_register16(reg, d, 4) != i2c::ERROR_OK);
}
...
{
  uint8_t value = 0;
  last_status = (this->io_->read_register16((uint16_t)reg, &value, 1) != i2c::ERROR_OK);
  return value;
}
...
{
  uint8_t b[2] = {0, 0};
  last_status = (this->io_->read_register16(reg, b, 2) != i2c::ERROR_OK);
...
{
  uint8_t b[4] = {0, 0, 0, 0};
  last_status = (this->io_->read_register16(reg, b, 4) != i2c::ERROR_OK);
...
{
  // Block-read the 17-byte result buffer in a single transaction (ESPHome i2c).
  uint8_t b[17] = {0};
  last_status = (this->io_->read_register16((uint16_t)RESULT__RANGE_STATUS, b, 17) != i2c::ERROR_OK);

  results.range_status = b[0];
  // b[1] report_status: not used
  results.stream_count = b[2];
  results.dss_actual_effective_spads_sd0 = ((uint16_t)b[3] << 8) | b[4];
  // b[5..6] peak_signal_count_rate_mcps_sd0: not used
  results.ambient_count_rate_mcps_sd0 = ((uint16_t)b[7] << 8) | b[8];
  // b[9..10] sigma_sd0, b[11..12] phase_sd0: not used
  results.final_crosstalk_corrected_range_mm_sd0 = ((uint16_t)b[13] << 8) | b[14];
  results.peak_signal_count_rate_crosstalk_corrected_mcps_sd0 = ((uint16_t)b[15] << 8) | b[16];
}
