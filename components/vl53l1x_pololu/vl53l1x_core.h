...
// Vendored from Pololu's vl53l1x-arduino library (BSD-3-Clause; (c) ST + Pololu).
// Only the transport layer is changed: Arduino Wire -> ESPHome i2c::I2CDevice.
// See LICENSE.txt in this directory. Register/ranging logic is byte-for-byte ST/Pololu.
#include <cstdint>
#include "esphome/components/i2c/i2c.h"
#include "esphome/core/hal.h"  // esphome::millis/delay/delayMicroseconds (resolved via enclosing namespace)
...
class VL53L1X
{
  public:
   ...
   // RangeInvalid              =  14: can't actually be returned by API (range can never become negative, even after correction)
    ...
    // Exposed for diagnostics: the per-frame stream counter (advances each ranging).
    uint8_t getStreamCount() { return results.stream_count; }

    uint8_t last_status; // status of last I2C transmission

    VL53L1X();

    void set_i2c(i2c::I2CDevice *io) { this->io_ = io; }
